...
class JobSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'https://www.lagou.com/gongsi/{}/'.format(i) for i in range(1, 20)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse, headers=self.headers)

    def parse(self, response):
        for company in response.css('li.company-item div.top'):
            yield {
                'name':company.css('p.company-name a::text').extract_first(),
                'image_url':company.xpath('.//p/a/img/@src').extract_first(),
                ...
                'about': company.css('p.advantage::text').extract_first()
            }
